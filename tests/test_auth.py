...
def test_validation_errors(client):
    ...
    response = client.post("/auth/register", {"username": ["bad"], "password": "secret1"})
    assert response.status_code == 400

    response = client.post("/auth/register", {"username": "alice", "password": ["bad"]})
    assert response.status_code == 400

    response = client.post("/auth/login", {})
    assert response.status_code == 400

    response = client.post("/auth/login", {"username": "alice", "password": ["bad"]})
    assert response.status_code == 400
